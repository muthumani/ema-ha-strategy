...
This script runs a single backtest with both sequential and parallel implementations
...
# Set up logger
logger = setup_logger(name="backtest.validate", log_level=logging.INFO)

def run_single_test(ema_short: int, ema_long: int, trading_mode: str, pattern: str,
                   config: Dict[str, Any], data: pd.DataFrame, seed: int = 42) -> Dict[str, Any]:
    """
    Run a single backtest with fixed parameters and seed
...
def validate_results(config_path: str, data_path: str = None, seed: int = 42):
    """
    Run a single backtest with both sequential and parallel implementations
    and compare the results to ensure they are identical.
    ...
    # Run sequential test
    logger.info(f"Running sequential test for EMA {ema_short}/{ema_long} with {trading_mode} mode and {pattern} pattern...")
    sequential_start = time.time()
    sequential_result = run_single_test(ema_short, ema_long, trading_mode, pattern, config, data, seed)
    sequential_end = time.time()
    sequential_time = sequential_end - sequential_start

    # Save sequential result
    with open(output_dir / 'sequential_single_result.json', 'w') as f:
        json.dump(sequential_result, f, indent=4)

    # Import deterministic backtest here to avoid circular imports
    from backtest.deterministic import DeterministicBacktest

    # Run deterministic test
    logger.info(f"Running deterministic test for EMA {ema_short}/{ema_long} with {trading_mode} mode and {pattern} pattern...")
    ...
    with open(output_dir / 'deterministic_single_result.json', 'w') as f:
    ...
    with open(output_dir / 'single_differences.json', 'w') as f:
    ...
    if identical:
        ...
        logger.error(f"See {output_dir / 'single_differences.json'} for details")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Validate sequential and deterministic backtest implementations")
    ...
    validate_results(args.config, args.data, args.seed)
